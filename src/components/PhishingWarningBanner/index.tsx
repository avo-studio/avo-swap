...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://swap.avostudio.io - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/swap.avostudio.io)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://swap.avostudio.io'}
          color={text === 'https://swap.avostudio.io' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
